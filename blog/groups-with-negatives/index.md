...
[*Update Nov 21, 2025: This "definition" is actually ill-defined, since group objects are only defined in a cartesian monoidal category, and the monoidal category in question is not cartesian. However, a _monoid_ with negation is indeed a monoid object in the category of $$\mathbb{Z}/2$$ sets; see [my latest article](../groups-with-roots-of-unity/).]

[**Update Nov 17, 2025: This is false if $$K$$ has characteristic 2. In that case, everything is already its own negative, so we just get the group algebra, which is $$\lvert G\rvert$$-dimensional.]

[***Update Nov 21, 2025: This last statement, which attempts to generalize my earlier ill-defined statement, is of course ill-defined. However, it is correct if "group" is replaced with "monoid". (It is also correct if $$n=1$$, in which case it is saying that a group is a group object in the category of sets.) Although the statement is ill-defined, there is a sense it which it is closer to being true for $$n=2$$ than for $$n > 2$$. The reason is that the inversion map of a group object in a category must be a morphism in the category. If we take a group with negation $$(G,\xi)$$ and make it into a $$\mathbb{Z}/2$$-set in the obvious way &mdash; $$k\in\mathbb{Z}/2$$ acts as multiplication by $$\xi^k$$ &mdash; then the inversion map of $$G$$ is indeed a morphism of $$\mathbb{Z}/2$$-sets: for all $$k\in\mathbb{Z}/2$$ and $$x\in G$$, we have 

$$(\xi^k x)^{-1} = x^{-1}\xi^{-k} = \xi^{-k}x^{-1} = \xi^{k}x^{-1}.$$ 

This was part of my thought process when I wrote "definition 3." However, if the order of $$\xi$$ were anything other than 1 or 2, the final step above would fail (for some values of $$k$$), since $$\xi\neq\xi^{-1}$$. So for $$n > 2$$, a group with $$n$$th root of unity cannot possibly be equivalent to a "group object in the category of $$\mathbb{Z}/n$$-sets", even if there is some way to make sense of the latter.]