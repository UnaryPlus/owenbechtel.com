class DarkMode extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(300, 100, "dark", fontStyle).setOrigin(0.5, 0.5)
        ...
        costumeSelectButton.once("pointerup", () => {
            this.scene.start("costume-select", { sceneNumber: 1, levelNumber: "dark-mode" })
        })
        ...
        howToPlayButton.once("pointerup", () => {
            this.scene.start("how-to-play", "dark-mode")
        })
        ...
        if(data.scores.darkMode.length > 0) {
            //calculate best, previous, and average scores
            const best = data.scores.darkMode.reduce((a, b) => Math.max(a, b), 0)
            const previous = data.scores.darkMode[data.scores.darkMode.length - 1]
            const average = data.scores.darkMode.reduce((a, b) => a + b, 0) / data.scores.darkMode.length
        ...
        //start game and create shrinking window when key is pressed
        this.started = false
        this.gameOver = false
        onStart(() => {
            this.started = true
            this.window = this.add.image(this.player.x, this.player.y, "window")
            .setScale(8)
            .setDepth(0.5)
            this.tweens.add({
                targets: this.window,
    ...
    update() {
        ...
        //make window follow player so that you can see only what's nearby
        this.window.setPosition(this.player.x, this.player.y)
        ...
        if(this.cameras.main.scrollY > this.player.y + 30 && !this.gameOver) {
            ...
            data.scores.darkMode.push(this.player.y)
