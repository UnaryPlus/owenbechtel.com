class EndlessMode extends Phaser.Scene {
    constructor() {
        super("endless-mode")
    }

    create() {
        //create endless mode text
        this.add.text(300, 100, "endless", fontStyle).setOrigin(0.5, 0.5)
        ...
        costumeSelectButton.once("pointerup", () => {
            this.scene.start("costume-select", { sceneNumber: 1, levelNumber: "endless-mode" })
        })
        ...
        howToPlayButton.once("pointerup", () => {
            this.scene.start("how-to-play", "endless-mode")
        })
        ...
        if(data.scores.endlessMode.length > 0) {
            //calculate best, previous, and average scores
            const best = data.scores.endlessMode.reduce((a, b) => Math.max(a, b), 0)
            const previous = data.scores.endlessMode[data.scores.endlessMode.length - 1]
            const average = data.scores.endlessMode.reduce((a, b) => a + b, 0) / data.scores.endlessMode.length
    ...
    update() {
        ...
        if(this.cameras.main.scrollY > this.player.y + 30 && !this.gameOver) {
            ...
            data.scores.endlessMode.push(this.player.y)
